...
void RLController::LoadModel() {
  // create env
  onnx_env_ = std::make_shared<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "LeggedOnnxController");
  Ort::SessionOptions sessionOptions;
  sessionOptions.SetInterOpNumThreads(1);
  session_ptr_ = std::make_unique<Ort::Session>(*onnx_env_, onnx_conf_.policy_file.c_str(), sessionOptions);

...
**AimRT application** (not standalone ROS 2) — all 5 modules are AimRT plugins loaded from `libpkg1.so` via `pkg/pkg1/pkg_main.cc`.
...
`control_module/policy/rl_walk.onnx` — 36-dim obs (lin vel 3, ang vel 3, proj gravity 3, joint pos 8, joint vel 8, last act 8, height 1, cmd counter 2) → 8-dim action (delta joint positions). Inference runs every 4th control cycle (effective 250 Hz at 1 kHz loop).
...
- `README.md` is empty — no user-facing documentation exists
